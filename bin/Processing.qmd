...
This Notebook aims to explain all the steps done by us for cleaning and transforming the data from the `cardiovascular_disease.csv` file. This dataset consists of 70,000 patient data points. Below is the information for each attribute:
...
multiple_model <- glm(data = df_cleaned, cardio ~ age_year +
                        gender +
                        height +
                        weight +
                        ap_hi +
...
All of the columns seem to be significant, except `$gender`,`$pulse`,`$bmi`

### 4.1 Visualizing the Most significant columns

```{r}
## Plot systolic pressure by cardio and cholesterol in the logistic regression
ggplot(df_cleaned, aes(x = ap_hi, y = cardio, color = cholesterol)) +
...
ggsave(filename = "GLMModel_PredictedProb.png",height = 15,width = 20,units = "cm")
